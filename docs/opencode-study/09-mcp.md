...
### 11.5 实战辨析（B 步补充，2026-08-24）：Skill 含脚本能否替代 MCP？——"电话 vs 通话"

> 本节是实际调研后补上的**判断框架**，用于回答"什么时候该用 Skill、什么时候该用 MCP"。触发自一个真实观察：全局 skill `imini-generate` 以前以 MCP 方式提供（要写 MCP Client 代码对接），现在迁成 Skill（`SKILL.md` + `scripts/*.py` 直连 REST API），使用者零代码、装即用。

#### ① Skill 是怎么"替代"MCP 的（机制）

**Skill 不直接给模型能力，它是教模型"怎么用自己的工具（bash）去做事"。**

```
MCP 方式：模型调 MCP 工具 → Server 执行 → 返回结果
            └─ 需要：Client + Server + 握手 + 生命周期 + 权限门
Skill 方式：模型读 SKILL.md → 学到"跑 generate_image.py" → 调 bash 工具执行
            └─ 复用现有 bash 工具，零新协议、零常驻进程、零运维
```

imini 的结构就是活例子：`SKILL.md`（操作手册）+ `scripts/`（直连 `openapi.imini.ai` 的 REST，含认证/轮询/错误处理）+ `references/`（模型目录、工作流）。**"对接云端服务"这件事被降级成了"一个脚本 + 一份说明书"。**

#### ② 核心判断框架：电话 vs 通话

> **这服务是"打一个电话就够"，还是"要一直保持通话"？**

- **打一个电话**（查一下 / 算一下 / 生成一下 / 调一次 API）→ **Skill**。不需要常驻、不需要订阅、不需要权限谈判，脚本拿起话筒拨号就行。
- **一直保持通话**（实时消息 / 持续操作一个活程序 / 长时间协作）→ **MCP**。保持通话需要"这条线一直通着"——那正是 MCP Server 常驻进程干的事。

#### ③ 五个问题清单（更细的判断路径）

| 问题 | Skill（脚本+说明书） | MCP |
|---|---|---|
| 1. 它是"一个接口"还是"一个活程序"？ | 接口（REST） | **活程序**（常驻/有状态） |
| 2. 数据是我去查的，还是会**主动推给我**的？ | 查（轮询即可） | **推**（高频/无法预知，轮询不动） |
| 3. 只有我用，还是**很多人/很多工具**用？ | 自己 | **共享**（公共基础设施，一次实现 N 端复用） |
| 4. 需要**精细权限边界**（作用域/OAuth scope）吗？ | 一般不需要（一个 key 够） | **需要**（协议层管） |
| 5. 服务需要**反过来借用宿主模型**（Sampling）吗？ | 不能 | 能 |

**全部答"否"→ Skill 更香；任何一条命中"是"→ 倾向 MCP。**

#### ④ 案例速查（练直觉）

| 服务 | 电话/通话？ | 判断 | 理由 |
|---|---|---|---|
| 接微信 | 通话 | **MCP** | 活程序 + 事件推送 + 精细权限 |
| Notion 只读查文档 | 电话 | **Skill** | REST + key + 无状态，同 imini |
| Notion 团队实时协作 | 通话 | **MCP** | 要订阅变更 + OAuth scope |
| GitHub 偶尔开 PR | 电话 | **Skill** | 有 REST API，一次调用 |
| 浏览器自动化（AI 操作网页） | 通话 | **MCP** | 操作活浏览器，有状态 |
| 天气查询 | 电话 | **Skill** | 简单 REST |
| 数据库持续分析会话 | 通话 | **MCP** | 保持连接/会话 |
| 翻译一句话 | 电话 | **Skill** | 简单 API |
| 飞书/钉钉群机器人发消息 | 边缘 | 看需求 | 偶尔发→Skill；实时收发→MCP |

**规律**：越是"一次性的查/算/生成"→ Skill；越是"持续连接、实时、有状态、多人共享"→ MCP。

#### ⑤ 一个诚实的提醒

**"长任务"未必需要 MCP**（imini 的轮询就够，本质是"提交任务→查状态"）。需要 MCP 的是"**事件无法预知、且来得频繁**"——轮询要么白烧请求、要么延迟拖沓，实时流（股价/消息/协作光标）根本轮询不动。这恰恰印证判断框架：**要"打一个电话"的（哪怕电话那头很慢）用 Skill；要"一直通着、随时被喊"的用 MCP。**

> 综合结论：**Skill 替代 MCP 成立的前提是"这个服务的本质是知识/接口，不需要活的执行边界"**（REST + key + 无状态 + 自己用）。反之，活的/推送的/共享的/细权限的/要借模型的——MCP 不可替代。
